...
export async function GET(request) {
  ...
  try {
    ...
    let tags = await allQuery(dataQuery, params);
...
export async function POST(request) {
  ...
  try {
    ...
    const { action, tagId, name, heat, clickCount, locale } = body;
    ...
    try {
      if (action === 'create') {
        ...
        if (name.includes(',') || name.includes('，')) {
          await rollbackTransaction(conn);
          return NextResponse.json({ success: false, error: 'Tag name cannot contain commas' }, { status: 400 });
        }

        const tagLocale = locale || (localeFilter ? localeFilter[0] : 'en');
        
        const existing = await getQuery('SELECT id FROM tags WHERE name = $1 AND locale = $2', [name, tagLocale]);
        if (existing) {
          await rollbackTransaction(conn);
          return NextResponse.json({ success: false, error: 'Tag with this name already exists in this locale' }, { status: 400 });
        }

        const result = await smartInsert('tags', {
          name,
          heat: heat || 0,
      ...
      if (action === 'update') {
        ...
        const existingTag = await getQuery('SELECT locale FROM tags WHERE id = $1', [tagId]);
        if (!existingTag) {
          await rollbackTransaction(conn);
          return NextResponse.json({ success: false, error: 'Tag not found' }, { status: 404 });
        }

        if (localeFilter && !localeFilter.includes(existingTag.locale)) {
          await rollbackTransaction(conn);
          return NextResponse.json({ 
            success: false, 
            error: `You do not have permission to manage locale: ${existingTag.locale}` 
          }, { status: 403 });
        }
        ...
        await smartUpdate('tags', updateData, 'id = $1', [tagId]);
      ...
      if (action === 'delete') {
        ...
        await executeQuery('DELETE FROM tags WHERE id = $1', [tagId]);
