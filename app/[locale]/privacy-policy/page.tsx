...
function getLocaleFromHeaders(headers: Headers): string {
  ...
  const url = new URL(referer || 'http://localhost:3000/en');
  const pathParts = url.pathname.split('/').filter(Boolean);
  const locale = pathParts[0];
  return supportedLocales.includes(locale as any) ? locale : 'en';
}

export default function PrivacyPolicyPage({ params }: { params: { locale: string } }) {
  const locale = params.locale || 'en';
  const content = staticPagesContent.legal.privacyPolicy[locale as keyof typeof staticPagesContent.legal.privacyPolicy] 
    || staticPagesContent.legal.privacyPolicy.en;
