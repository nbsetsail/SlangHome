...
export const syncToRedis = async (redis, config) => {
  ...
  console.log(`📤 Syncing memory cache to Redis (${entries.length} items)...`);
  ...
  for (const { key, value, remainingTTL } of entries) {
    try {
      ...
      failed++;
      console.error(`Cache sync failed [${key}]:`, err.message);
    }
  }

  console.log(`✅ Memory cache synced: ${synced} succeeded, ${failed} failed`);
