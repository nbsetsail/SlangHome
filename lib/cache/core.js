...
export const warmupCache = async (locale = 'en') => {
  ...
  try {
    ...
    console.log('  Warming up system configs...');
    ...
    console.log('  Warming up hot slang...');
    const hotSlang = await allQuery(`
      ...
      LIMIT 50
    `, [locale]);
    await cacheSet(cacheKeys.slang.hot(locale), hotSlang, cacheTTLs.hot());

    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    console.log(`✅ Cache warmup completed - ${duration}s`);
    console.log(`   Locale: ${locale}`);
    console.log(`   Categories: ${categories.length} items`);
    console.log(`   Tags: ${tags.length} items`);
    console.log(`   Hot slang: ${hotSlang.length} items`);

    return { categories, tags, hotSlang };
  } catch (error) {
