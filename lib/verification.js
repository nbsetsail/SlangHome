...
export async function validateVerificationCode(db, email, code) {
  ...
  const result = await db.query(`
    ...
    WHERE email = $1 AND code = $2 AND used = 0 AND expires_at > $3
    LIMIT 1
  `, [email, code, now]);
...
export async function invalidateActiveVerificationCodes(db, email) {
  ...
  const result = await db.query(`
    UPDATE email_verification_codes
    SET expires_at = $1
    WHERE email = $2 AND used = 0 AND expires_at > $3
  `, [now, email, now]);
...
export async function getVerificationStats(db, email) {
  ...
  const result = await db.query(`
    SELECT 
      ...
      SUM(CASE WHEN used = 0 AND expires_at > $1 THEN 1 ELSE 0 END) as active_codes
    FROM email_verification_codes
    WHERE email = $2
  `, [now, email]);
